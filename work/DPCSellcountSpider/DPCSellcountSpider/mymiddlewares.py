...
class RotateUserAgentMiddleware(UserAgentMiddleware):
    ...
    #     return cls(crawler.user_agent_list)
    @classmethod
...
class ProxyMiddleware(object):
    ...
    # 代理池  wgh change 2017-4-7 5个代理池 随机选择某一个
    proxykeys = ["proxy:iplist", "proxy:iplist2"]
    ...
    def process_exception(self, request, exception, spider):
        ...
        print 'proxy ip is error ------------------', exception
        try:
        ...
        try:
            ...
            if len(request.meta['sellcountUrl']) > 15:
                proip = random.choice(self.proxykeys)
                self.redisclient = redis.Redis(self.REDIS_HOST, self.REDIS_PORT)
                proxy = self.redisclient.srandmember(proip)
                proxyjson = json.loads(proxy)
                ip = proxyjson["ip"]
